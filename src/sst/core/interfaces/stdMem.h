...
#define PRI_ADDR PRIx64
    ...
    public:
        FlushCache(
            bool inv = true, uint32_t depth = std::numeric_limits<uint32_t>::max(), flags_t flags = 0,
            Addr instPtr = 0, uint32_t tid = 0) :
            Request(flags),
            inv(inv),
            depth(depth),
        ...
        {
            std::ostringstream str;
            str << "ID: " << id << ", Type: FlushCache, Flags: [" << getFlagString() << "], Inv: " << (inv ? "T" : "F"); 
            str << ", Depth: " << std::dec << depth << ", InstPtr: 0x" << std::hex << iPtr << ", ThreadID: " << std::dec << tid;
            return str.str();
        }
