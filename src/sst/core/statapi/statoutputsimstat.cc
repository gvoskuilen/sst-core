...
#include "sst/core/statapi/statoutputsimstat.h"
...
StatisticOutputSimStat::StatisticOutputSimStat(Params& outputParameters)
    : StatisticFieldsOutput (outputParameters)
{
    // Announce this output object's name
    Output &out = Simulation::getSimulationOutput();
    out.verbose(CALL_INFO, 1, 0, " : StatisticOutputSimStat enabled...\n");
    setStatisticOutputName("StatisticOutputSimStat");
...
bool StatisticOutputSimStat::checkOutputParameters()
{
    bool foundKey;
    ...
    m_verboseOutput = getOutputParameters().find<bool>("verbose", true);
...
void StatisticOutputSimStat::printUsage()
{
    // Display how to use this output object
    Output out("", 0, 0, Output::STDOUT);
    out.output(" : Usage - Sends all statistic output to a JSON File according to the SimStat format.\n");
...
void StatisticOutputSimStat::startOfSimulation()
{
    ...
    printIndent();
    fprintf(m_hFile, "\"type\" : \"SSTSimulator\", \n");

}

void StatisticOutputSimStat::endOfSimulation()
{
...
void StatisticOutputSimStat::implStartOutputEntries(StatisticBase* statistic)
{
    if( m_currentComponentName != statistic->getCompName() ) {
        ...
        printIndent();
        fprintf(m_hFile, "\"type\" : \"%s\",\n", statistic->getCompType().c_str());
    ...
    printIndent();
    fprintf(m_hFile, "\"%s\" : { \n", statistic->getStatName().c_str());

    m_curIndentLevel++;
    printIndent();
    fprintf(m_hFile, "\"type\" : \"%s\", \n", m_currentStatisticType.c_str());

    printIndent();
    fprintf(m_hFile, "\"value\" : [ ");

    m_processedAnyStats = true;
    m_firstField = true;
}

void StatisticOutputSimStat::implStopOutputEntries()
{
    fprintf(m_hFile, " ] \n");
    m_curIndentLevel--;
    printIndent();
...
void StatisticOutputSimStat::outputField(fieldHandle_t UNUSED(fieldHandle), int32_t data)
{
    if( ! m_firstField) {
        fprintf(m_hFile, ", ");
    }
...
void StatisticOutputSimStat::outputField(fieldHandle_t UNUSED(fieldHandle), uint32_t data)
{
    if( ! m_firstField) {
        fprintf(m_hFile, ", ");
    }
...
void StatisticOutputSimStat::outputField(fieldHandle_t UNUSED(fieldHandle), int64_t data)
{
    if( ! m_firstField) {
        fprintf(m_hFile, ", ");
    }
...
void StatisticOutputSimStat::outputField(fieldHandle_t UNUSED(fieldHandle), uint64_t data)
{
    if( ! m_firstField) {
        fprintf(m_hFile, ", ");
    }
...
void StatisticOutputSimStat::outputField(fieldHandle_t UNUSED(fieldHandle), float data)
{
    if( ! m_firstField) {
        fprintf(m_hFile, ", ");
    }
...
void StatisticOutputSimStat::outputField(fieldHandle_t UNUSED(fieldHandle), double data)
{
    if( ! m_firstField) {
        fprintf(m_hFile, ", ");
    }
...
bool StatisticOutputSimStat::openFile(void)
{
    ...
    if (nullptr == m_hFile) {
        ...
        out.fatal(CALL_INFO, 1, " : StatisticOutputSimStat - Problem opening File %s - %s\n", m_FilePath.c_str(), strerror(errno));
        return false;
    }
...
void StatisticOutputSimStat::closeFile(void)
{
    fclose(m_hFile);
}

void StatisticOutputSimStat::printIndent() {
